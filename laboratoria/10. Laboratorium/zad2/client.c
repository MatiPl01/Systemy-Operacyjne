...
void connect_to_server(struct sockaddr* addr, socklen_t addr_size);
...
void sigint_handler(int sig_no) {
    ...
    catch_perror(sendto(client_socket_fd, &msg, sizeof msg, 0, NULL, sizeof(struct sockaddr_in)));
    exit(EXIT_SUCCESS);
}
...
void connect_to_local_server(void) {
    struct sockaddr_un local_server_addr, local_server_bind_addr;
    local_server_addr.sun_family = AF_UNIX;
    local_server_bind_addr.sun_family = AF_UNIX;
    sprintf(local_server_bind_addr.sun_path, "/tmp/%s%d", nickname, getpid());
    strcpy(local_server_addr.sun_path, server_address);
    client_socket_fd = catch_perror(socket(AF_UNIX, SOCK_DGRAM, 0));
    catch_perror(bind(client_socket_fd, (struct sockaddr*) &local_server_bind_addr, sizeof local_server_bind_addr));
    // Open the network server socket and establish connection
    connect_to_server((struct sockaddr*) &local_server_addr, sizeof local_server_addr);
}
...
void connect_to_network_server(void) {
    ...
    // Open the network server socket and establish connection
    client_socket_fd = catch_perror(socket(AF_INET, SOCK_DGRAM, 0));
    connect_to_server((struct sockaddr*) &network_server_addr, sizeof network_server_addr);
}
...
void connect_to_server(struct sockaddr* addr, socklen_t addr_size) {
    ...
    cinfo("Sending the registration message to the server...\n");
    message msg = { .type = REGISTER };
    strcpy(msg.data.nickname, nickname);
    catch_perror(send(client_socket_fd, &msg, sizeof msg, 0));
}
...
void handle_response_event(void) {
    ...
    message msg;
    catch_perror(recvfrom(client_socket_fd, &msg, sizeof msg, 0, NULL, NULL));
...
void handle_ping_msg(message *msg) {
    ...
    catch_perror(write(client_socket_fd, msg, sizeof *msg));   // TODO - check if this works
}

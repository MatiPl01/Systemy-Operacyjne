...
#define PRODUCER_DIR_PATH "./files/producer"
...
char* PRODUCER_FILES[] = {
        ...
        PRODUCER_DIR_PATH"/text-file-5.txt"
};
...
void print_centered(char* text, size_t width, char fill_char);
pid_t exec_in_child(char* args[]);
int wait_for_processes(pid_t *pids, int no_processes, char* process_name);


int main(int argc, char* argv[]) {
    if (argc > 3) {
        fprintf(stderr, "main: Too many arguments. Expected %d or %d, got %d.\n", 2, 3, argc - 1);
        return EXIT_FAILURE;
    }
    ...
    if (no_producers <= 0 || no_producers > max_no_producers) {
        fprintf(stderr, "main: Number of producers must be a positive integer not greater than %d.\n", max_no_producers);
        return EXIT_FAILURE;
    }

    if ((no_consumers = get_input_num(&i, argc, argv, "Please provide a number of consumers")) <= 0) {
        fprintf(stderr, "main: Number of consumers must be a positive integer.\n");
        return EXIT_FAILURE;
    }

    if (mkfifo(FIFO_PATH, 0777) == -1) {
        if (errno != EEXIST) {
            perror("main: Unable to create a fifo.\n");
            return EXIT_FAILURE;
        }
    }

    switch (get_mode(no_consumers, no_producers)) {
        case MANY_TO_ONE: return many_to_one(no_producers) == -1 ? EXIT_FAILURE : EXIT_SUCCESS;
        case ONE_TO_MANY: return one_to_many(no_consumers) == -1 ? EXIT_FAILURE : EXIT_SUCCESS;
        case MANY_TO_MANY: return many_to_many(no_producers, no_consumers) == -1 ? EXIT_FAILURE : EXIT_SUCCESS;
...
int get_mode(int no_consumers, int no_producers) {
    ...
    fprintf(stderr, "main: Unable to find a suitable mode. Wrong number of consumers or producers.\n");
    return -1;
}
...
void print_centered(char* text, size_t width, char fill_char) {
    ...
    for (size_t i = 0; i < l_just; i++) printf("%c", fill_char);
    printf("%s", text);
    for (size_t i = 0; i < width - length - l_just; i++) printf("%c", fill_char);
    printf("\n");
}

int many_to_one(int no_producers) {
    print_centered("Many producers to one consumer", 30, '=');
    ...
    return wait_for_processes(pids, no_producers, "producer");
}
...
int one_to_many(int no_consumers) {
    ...
    return wait_for_processes(pids, no_consumers, "consumer");
}
...
int many_to_many(int no_producers, int no_consumers) {
    ...
    // Create consumer processes
    char *consumer_args[] = { CONSUMER_EXE_PATH, FIFO_PATH, CONSUMER_FILES[2], CONSUMER_READ_COUNT, NULL };
    ...
    int consumers_status = wait_for_processes(consumer_pids, no_consumers, "consumer");
    int producers_status = wait_for_processes(consumer_pids, no_producers, "producer");
...
pid_t exec_in_child(char* args[]) {
    ...
    if (pid < 0) {
        perror("main: Unable to create a child process.\n");
        return -1;
    }

    if (pid == 0) {
        if (execvp(args[0], args) == -1) {
            perror("main: Issues while executing a command.\n");
            exit(1);
        }
        exit(0);
    }
...
int wait_for_processes(pid_t *pids, int no_processes, char* process_name) {
    ...
    for (int i = 0; i < no_processes; i++) {
        if (waitpid(pids[i], &status, WNOHANG) == -1) {
            fprintf(stderr, "main: Unable to wait for a %s process with PID %d.\n", process_name, pids[i]);
            return -1;
        }
        // Check if a process finished with an error status code
        if (WIFEXITED(status) && WEXITSTATUS(status)) {
            fprintf(stderr, "main: There was an error in a child process with PID %d\n", pids[i]);
            return -1;
        }
    }
    return 0;

...
void* santa(void* args) {
    ...
    while (true) {
        ...
        pthread_mutex_lock(santa_args->reindeer_delivery_mutex);
        if (*(santa_args->reindeer_waining_count) == santa_args->total_reindeer_count) {
            deliver_presents(rand_context, santa_args);
        }
        pthread_mutex_unlock(santa_args->reindeer_delivery_mutex);

        if (santa_args->deliveries_count == santa_args->max_deliveries_count) {
            printcf(SANTA_MSG_COLOR, "Mikołaj: Mam dość na dziś, ignoruję kolejne próby obudzenia mnie!\n");
            break;
        }
        ...
        if (*(santa_args->elves_waiting_for_help_count) == santa_args->max_elves_waiting_for_help) {
...
static void deliver_presents(struct drand48_data rand_context, santa_args_t *santa_args) {
    ...
    randsleep(&rand_context, santa_args->min_delivery_duration, santa_args->max_delivery_duration);
...
static void solve_elves_problem(struct drand48_data rand_context, santa_args_t *santa_args) {
    ...
    pthread_mutex_lock(santa_args->santa_started_solving_problem_mutex);
    ...
    puts("");
    pthread_mutex_unlock(santa_args->santa_started_solving_problem_mutex);

    randsleep(&rand_context, santa_args->min_problem_solving_duration, santa_args->max_problem_solving_duration);
